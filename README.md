...
> Implementacja: `src/compiler/parser/Proto3.g4` (gramatyka ANTLR4)
...
```bnf
<syntaxStatement>  ::= SYNTAX EQ STRING_LITERAL SEMI

<importStatement>  ::= IMPORT <optImportMod> STRING_LITERAL SEMI
...
<optionName>       ::= <optionNamePart> <optionNameTail>
<optionNamePart>   ::= IDENTIFIER | LPAREN DOT? <fullIdent> RPAREN
<optionNameTail>   ::= DOT <optionNamePart> <optionNameTail> | ε

<typeReference>    ::= <optDot> IDENTIFIER <typeRefTail>
<optDot>           ::= DOT | ε
<typeRefTail>      ::= DOT IDENTIFIER <typeRefTail> | ε
```
...
<type>             ::= <scalarType> | <typeReference>

<scalarType>       ::= DOUBLE | FLOAT | INT32 | INT64 | UINT32 | UINT64
...
```bnf
<field>            ::= <optFieldMod> <type> IDENTIFIER EQ <fieldNumber>
                       <optFieldOpts> SEMI
<optFieldMod>      ::= REPEATED | OPTIONAL | ε
<optFieldOpts>     ::= LBRACKET <fieldOptions> RBRACKET | ε
...
<oneofDef>         ::= ONEOF IDENTIFIER LBRACE <oneofBody> RBRACE
<oneofBody>        ::= <oneofItem> <oneofBody> | ε
<oneofItem>        ::= <optionStatement> | <oneofField> | <emptyStatement>
<oneofField>       ::= <type> IDENTIFIER EQ <fieldNumber>
                       <optFieldOpts> SEMI

<mapField>         ::= MAP LANGLE <keyType> COMMA <type> RANGLE
                       IDENTIFIER EQ <fieldNumber>
                       <optFieldOpts> SEMI
```
...
```bnf
<reserved>         ::= RESERVED <reservedBody> SEMI
...
<messageBody>      ::= LBRACE <msgBodyList> RBRACE
...
<enumBody>         ::= LBRACE <enumBodyList> RBRACE
<enumBodyList>     ::= <enumBodyItem> <enumBodyList> | ε
<enumBodyItem>     ::= <optionStatement> | <enumField>
                     | <reserved> | <emptyStatement>
<enumField>        ::= IDENTIFIER EQ <optMinus> INTEGER_LITERAL
                       <optEnumValOpts> SEMI
<optMinus>         ::= MINUS | ε
<optEnumValOpts>   ::= LBRACKET <enumValOptList> RBRACKET | ε
